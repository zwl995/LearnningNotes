...
- 指向非常量数据的非常量指针（```int* countPtr;```）： 具有最大的访问权限——可以通过简介引用指针使其修改数据，也可以修改指针使其指向其他数据。
- 指向常量数据的非常量指针（ ```const int* countPtr;```）： 可以被修改以指向任何适当类型的其他数据项，但是不能通过该指针来修改它所指向的数据。
- 指向非常量数据的常量指针（```int* const countPtr;```）： 始终指向同一个内存位置，通过该指针可以修改这个位置上的数据。这就是数组名的默认情况，数组名是指向这个数组开始处的常量指针。声明为const的指针在声明时必须被初始化。
- 指向常量数据的常量指针（```const int* const countPtr;```）： 总是指向内存中相同的位置，并且不能用该指针修改这个内存位置的数据。
